...
class EditPlayerProfileForm(forms.Form):
    """Simplified form for player transfers - only requires codename and destination team PIN"""
    
    # Authentication field - only codename needed
    codename = forms.CharField(
        max_length=6,
        min_length=6,
        widget=forms.PasswordInput(attrs={
        ...
        label='Your Codename',
        help_text='Enter your unique 6-character codename to identify yourself'
    )
    ...
    # Team change fields - simplified to only require destination team PIN
    change_team = forms.BooleanField(
        ...
        label='Transfer to New Team',
        help_text='Check this box if you want to transfer to a different team'
    )
    ...
    new_team_pin = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(attrs={
            'placeholder': 'Enter destination team PIN',
            'class': 'form-control'
        }),
        label='Destination Team PIN',
        help_text="Enter the PIN of the team you want to transfer to"
    )
    ...
    def clean(self):
        """Validate player authentication and team transfer logic"""
        cleaned_data = super().clean()
        codename = cleaned_data.get('codename')
        change_team = cleaned_data.get('change_team')
        new_team_pin = cleaned_data.get('new_team_pin')
        
        # Validate player authentication using only codename
        if codename:
            try:
                # Find the player by codename
                ...
                cleaned_data['authenticated_player'] = player
                
            except PlayerCodename.DoesNotExist:
                raise forms.ValidationError("Invalid codename. Please check and try again.")
        
        # Validate team transfer logic
        if change_team:
            if not new_team_pin:
                raise forms.ValidationError("Please enter the destination team PIN to complete the transfer.")
            
            # Validate that the destination team exists and PIN is correct
            try:
                from .models import Team
                destination_team = Team.objects.get(pin=new_team_pin)
                cleaned_data['destination_team'] = destination_team
            except Team.DoesNotExist:
                raise forms.ValidationError("Invalid team PIN. Please check and try again.")
