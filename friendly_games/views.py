...
def start_match(request, game_id):
    ...
    black_count = game.players.filter(team='BLACK').count()
    white_count = game.players.filter(team='WHITE').count()
    
    if black_count == 0 or white_count == 0:
        messages.error(request, 'Cannot start game: both teams must have at least one player.')
        return redirect('friendly_games:game_detail', game_id=game.id)
    ...
    messages.success(request, f'Game "{game.name}" has been started! Good luck!')
    return redirect('friendly_games:game_detail', game_id=game.id)
